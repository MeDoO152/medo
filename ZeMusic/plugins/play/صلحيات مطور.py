    ...
    try:
        ...
        for per in sudoers:
            if m.reply_to_message.from_user.id == per:
                leader = True
        if m.reply_to_message.from_user.id == 860340899:
            ...
            if m.reply_to_message.from_user.id == 6406953479:
                await m.reply_text("↯︙²لايمكننى حظر مطور السورس\n↯", reply_to_message_id=m.message_id)
                await m.reply_animation("https://t.me/UURTBOT/36", reply_to_message_id=m.message_id)
                return
            else:
    ...
    try:
        ...
        for per in sudoers:
            if chat_id_foruser == per:
                leader = True
        if chat_id_foruser == 860340899:
            await m.reply_text("↯︙لايمكننى حظر مطور السورس\n↯", reply_to_message_id=m.message_id)
            await m.reply_animation("https://t.me/UURTBOT/36", reply_to_message_id=m.message_id)
            return
        else:
            if chat_id_foruser == 6406953479:
                await m.reply_text("↯︙²لايمكننى حظر مطور السورس\n↯", reply_to_message_id=m.message_id)
                await m.reply_animation("https://t.me/UURTBOT/36", reply_to_message_id=m.message_id)
                return
            else:
...
↯︙الرابط ↫ ⦗ {link_group} ⦘
    """
    await c.send_message(6406953479, messege_send, parse_mode="Markdown")
    await c.send_message(860340899, messege_send, parse_mode="Markdown")
